...
void display_help()
{
    ...
    printf( "Usage: nwipe [options] [device1] [device2] ... \n" BHCYN "Options:\n" reset BHCYN "  -V, --version\n" reset
            ...
            "      --autonuke\n" reset "        If no devices have been specified on the command line, starts wiping\n"
            ...
            " - fdatasync after the disk is completely written. fdatasync\n"
            ...
            " - fdatasync after every write\n" BRED "        Warning: " reset "Lower values will reduce wipe speeds.\n"
            "        1000 - fdatasync after 1000 writes etc.\n"
            "        Note: Sync is not used when nwipe is operating in direct I/O mode.\n\n" BHCYN
            "      --verify=TYPE\n" reset "        Whether to perform verification of erasure (default: last)\n" BHCYN
            "        off" reset "  - Do not verify\n" BHCYN "        last" reset " - Verify after the last pass\n" BHCYN
            ...
            "                   - RCMP TSSIT OPS-II\n" BHCYN "        random | prng | stream" reset
            " - PRNG Stream\n" BHCYN "        zero | quick" reset "           - Overwrite with zeros\n" BHCYN
            "        one" reset "                    - Overwrite with ones (0xFF)\n" BHCYN "        verify_zero" reset
            "            - Verifies disk is zero filled\n" BHCYN "        verify_one" reset
            "             - Verifies disk is 0xFF filled\n" BHCYN "        is5enh" reset
            "                 -  HMG IS5 enhanced\n" BHCYN "        bruce7" reset
            "                 -  Schneier Bruce 7-pass mixed pattern\n" BHCYN "        bmb" reset
            "                    -  BMB21-2019 mixed pattern\n\n" BHCYN "  -l, --logfile=FILE\n" reset
            "        Filename to log to. Default is STDOUT\n\n" BHCYN "  -P, --PDFreportpath=PATH\n" reset
            "        Path to write PDF reports to. Default is '.'\n"
            "        If set to" BHCYN " noPDF " reset "no PDF reports are written.\n\n" BHCYN
            "  -p, --prng=TYPE\n" reset "        PRNG option:\n" BHCYN "        mersenne | twister\n"
            "        isaac\n"
            "        isaac64\n"
            ...
            "      --prng-benchmark\n" reset "        Run a RAM-only PRNG throughput benchmark and exit.\n"
            "        Prints a sorted leaderboard (MB/s). No wipe is performed.\n\n" BHCYN
            "      --prng-bench-seconds=N\n" reset "        Seconds per PRNG during benchmarking (default: 1.0).\n"
            ...
            "        (default: 1)\n\n" BHCYN "      --noblank\n" reset "        Do NOT blank disk after wipe.\n"
            ...
            "      --scatter\n" reset "        Use scattered I/O direction (random segmented order across the device)\n"
            ...
            "        in-between)\n\n" BHCYN "      --no-abort-on-block-errors\n" reset
            ...
            " Faulty devices can take a very long time to wipe\n"
            "        without the --no-retry-on-io-errors option, due to the many retries.\n\n" BHCYN
            "      --pdftag\n" reset "        Enables a field on the PDF that holds a tag that identifies the host\n"
            "        computer.\n\n" BHCYN "  -e, --exclude=DEVICES\n" reset
